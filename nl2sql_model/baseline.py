...
import codecs          #codecs专门用作编码转换，当然，其实通过它的接口是可以扩展到其他关于代码方面 的转换的，这个东西这里不涉及。
...
import jieba
import editdistance   #Given two words word1 and word2, find the minimum number of operations required to convert word1 to word2
import re
...
config_path = 'G:\\datas\\nl2sql\\chinese_wwm_L-12_H-768_A-12\\bert_config.json'
checkpoint_path = 'G:\\datas\\nl2sql\\chinese_wwm_L-12_H-768_A-12\\bert_model.ckpt'
dict_path = 'G:\\datas\\nl2sql\\chinese_wwm_L-12_H-768_A-12\\vocab.txt'
...
train_data, train_tables = read_data('G:\\datas\\nl2sql\\TableQA-master\\train\\train.tables.json', 'G:\\datas\\nl2sql\\TableQA-master\\train\\train.jso')
valid_data, valid_tables = read_data('G:\\datas\\nl2sql\\TableQA-master\\val\\val.tables.json', 'G:\\datas\\nl2sql\\TableQA-master\\val\\val.json')
test_data, test_tables = read_data('G:\\datas\\nl2sql\\TableQA-master\\test\\test.tables.json', 'G:\\datas\\nl2sql\\TableQA-master\\test\\test.json')
...
def seq_padding(X, padding=0, maxlen=None):   # 将每一句话补齐？  maxlen = 160
    if maxlen is None:
        L = [len(x) for x in X]
        ML = max(L)
    else:
    ...
    def __iter__(self):
        while True:
            ...
            for i in idxs:
                ...
                xm = [0] + [1] * len(d['question']) + [0]  #xm, hm, cm是三个mask变量，就是要去除训练过程中padding部分带来的效应
                h = []
                for j in t:
