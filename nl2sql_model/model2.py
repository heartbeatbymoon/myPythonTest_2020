...
import cn2an  # 中文数字与阿拉伯数字的相互转换
from tqdm import tqdm_notebook as tqdm  # 是一个快速,可扩展的Python进度条,可以在Python长循环中添加一个进度提示信息
from nl2sql_model.utils import read_data, read_tables, SQL, Query, Question, Table
...
def cn_to_an(string):
    try:
        return str(cn2an.cn2an(string, 'normal'))  # 中文转数字   一百 -> 100
    except ValueError:
        return string
...
def an_to_cn(string):
    try:
        return str(cn2an.an2cn(string))  # 讲一个数字转为中文的表达形式如“100” -> 一百
    except ValueError:
        return string


def str_to_num(string):  # 将一个数字、或者字符串转为数字，如果不是数字，return为None
    try:
...
def str_to_year(string):  # 见一个转为具体的年份， 如将“一零年”“10年”转为2010
...
class QuestionCondPair:  # 仅仅是为了显示对象用吗？为了生成最终的格式文件
    ...
    def __repr__(self):  # 内建函数str()和repr() (representation，表达，表示)或反引号操作符（``）可以方便地以字符串的方式获取对象的内容、类型、数值属性等信息。
        repr_str = ''
...
class CandidateCondsExtractor:
    ...
    def extract_year_from_text(self, text):  # 从文本中抽取年
        values = []
    ...
    def extract_num_from_text(self, text):  # 从文本中抽取数字
        values = []
    ...
    def extract_values_from_text(self, text):  # 从文本中抽取values
        values = []
        values += self.extract_year_from_text(text)
        values += self.extract_num_from_text(text)
        return list(set(values))

    def extract_values_from_column(self, query, col_ids):  # 按表中的列中抽取值
...
class QuestionCondPairsDataset:  # where 条件对
    """
...
# final output
final_output_file = 'final_output.json'  # 最终的sql。
